...
@interface ClientListViewController() <UITableViewDelegate,UITableViewDataSource, UINavigationControllerDelegate,UIImagePickerControllerDelegate, NSURLSessionDelegate>
...
-(void) removeObservers {
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"checkBadResend" object:nil];
...
-(void) showComeForegroundStatus:(NSString*)foregroundMessage {
    UIView *resendView = [[UIView alloc]initWithFrame:CGRectMake(40, 40, self.view.frame.size.width - 80, 120)];
    ...
    [updateNetworkLabel setFont:[UIFont fontWithName:@"Lato-Regular" size:20]];
    ...
    [updateNetworkLabel setText:foregroundMessage];
    [resendView addSubview:updateNetworkLabel];
    
    [UIView animateWithDuration:2.0 animations:^{
        resendView.alpha = 0.0;
    } completion:^(BOOL finished) {
        [resendView removeFromSuperview];
    }];
...
-(void) foregroundPollingUpdate {
    ...
    if ([self checkForBadResendBeforeSync]) {
        
        [self showComeForegroundStatus:@"RESENDING..."];
        
    } else {
    
        [self showComeForegroundStatus:@"UPDATING..."];

        @synchronized(@"updateToday") {
            [wTableView setUserInteractionEnabled:NO];
...
-(void) applicationEnterBackground {    
    ...
    for (DataClient *client in sharedVisits.clientData) {
        [client cleanDataClientData];
        if ([[client getPetInfo]count] > 0) {
            for (PetProfile *pet in [client getPetInfo]) {
                [pet removePhoto];
            }
        }
    }
...
- (void) addDateToView:(UIView *)dateView {
    
}
- (UIView*) tableView:(UITableView *)tableView
         viewForHeaderInSection:(NSInteger)section {

    headerView = [[UIView alloc] initWithFrame:CGRectMake(0,-40, tableView.frame.size.width, 100)];
    ...
    settingsButton.tag  = 300;

    UIImageView *mascotView = [[UIImageView alloc]initWithFrame:CGRectMake(5,0, 62 , 50)];
    [mascotView setImage:[UIImage systemImageNamed:@"square.and.pencil"]];
    //[mascotView setImage:[UIImage systemImageNamed:@"calendar" withConfiguration:[UIImageConfiguration ]]]
    //[UIImage imageNamed:@"icon-brand"]];
    mascotView.tag = 201;
...
- (UITableViewCell *) tableView:(UITableView *)tableView 
    ...
    __weak ClientListViewController *weakSelf = self;
...
-(void) tapDetailView:(id)sender {
    ...
    for (DataClient *clientProfile in sharedVisits.clientData) {
        if ([tapVisit.clientptr isEqualToString:clientProfile.clientID]) {
            ...
            detailAccordionView.view.tag = 4;
...
-(void) sendVisitReportNoButton:(NSString*) visitID {
    ...
    NSString *dateTimeString = [sharedVisits getTimeShortString:rightNow];
...
// ## PHOTO VIEW CONTROLLER AND RELATED PROTOCOL IMPLEMENTATIONS
...
-(void) goToPhotoTaker:(id)sender {
    ...
    if ([sender isKindOfClass:[UIButton class] ] ) {
        ...
        if ([self isCameraAvailable] && [self doesCameraSupportTakingPhotos]) {
            ...
            galleryButton.frame =CGRectMake(picker.view.frame.size.width - 140, picker.view.frame.size.height - 100, 60, 60);
            [galleryButton setBackgroundImage:[UIImage imageNamed:@"btnDefault"]
                                     forState:UIControlStateNormal];
            [galleryButton addTarget:self
                              action:@selector(pickPhotoFromPhotoCollection:)
                    forControlEvents:UIControlEventTouchUpInside];
            
            [picker.view addSubview:galleryButton];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    for (VisitDetails *visitDetail in sharedVisits.visitData) {
        if ([visitDetail.appointmentid isEqualToString:sharedVisits.onWhichVisitID]) {
            [visitDetail addImageForPet:(UIImage *)[info objectForKey:UIImagePickerControllerEditedImage]];
        }
    }
...
- (void) imagePickerControllerDidCancel:(UIImagePickerController *)picker {
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        NSArray *pickerViews = [picker.view subviews];
        for (int i = 0; i < [pickerViews count]; i++) {
            id pView = [pickerViews objectAtIndex:i];
            
            if ([pView isKindOfClass:[UIButton class]]) {
                UIButton *pButton = (UIButton*) pView;
...
-(BOOL) cameraSupportsMedia:(NSString *)paramMediaType sourceType:(UIImagePickerControllerSourceType)paramSourceType {
    ...
    for (NSString *type in availableMediaTypes) {
        NSLog(@"Type media available: %@", type);
    }
...
-(void) docErratButtonClick:(id)sender {
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        for(VisitDetails *visit in self->sharedVisits.visitData) {
            NSString *visitID = [NSString stringWithFormat:@"%li",(long)tapButton.tag];
            if ([visit.appointmentid isEqualToString:visitID]) {
                FloatingModalView *fmView = [[FloatingModalView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height, self.view.frame.size.width, self.view.frame.size.height) appointmentID:visitID itemType:@"oneDoc"];
                [fmView show];
            }
        }
...
-(void) markArriveButton:(id)sender {
    ...
    if([sender isKindOfClass:[UIButton class]]) {
        ...
        UIButton *statusButton = (UIButton*)sender;
        NSString *tagString = [NSString stringWithFormat:@"%li",(long)statusButton.tag];
        [statusButton setTitle:@"SENDING" forState:UIControlStateNormal];
        ...
        for(VisitDetails *visitInfo in sharedVisits.visitData) {
            ...
            if ([visitInfo.appointmentid isEqualToString:tagString] &&
                ...
                NSString *dateString2 = [dateTimeMarkArriveFormat stringFromDate:rightNow2];
...
-(void) markVisitComplete:(id)sender {
    if([sender isKindOfClass:[UIButton class]]) {
        ...
        for(VisitDetails *localVisit in sharedVisits.visitData) {
            if ([localVisit.appointmentid isEqual:tagString]) {
                if ([localVisit.status isEqualToString:@"completed"]) {
                    // ALREADY COMPLETED
                }
                else if ([localVisit.status isEqualToString:@"canceled"]) {
                    // CANCELED VISIT
                }
                else if ([localVisit.status isEqualToString:@"arrived"]) {
                    LocationTracker *locationTracker = [LocationTracker sharedLocationManager];
                    if (locationTracker.isLocationTracking && !sharedVisits.multiVisitArrive) {
                        [locationTracker stopLocationTracking];
                    }
                    
                    localVisit.completed = dateTimeString;
                    localVisit.NSDateMarkComplete = rightNow;
                    localVisit.dateTimeMarkComplete = dateTimeString;
                    localVisit.isComplete = YES;
                    localVisit.status = @"completed";
                    
                    sharedVisits.onWhichVisitID = @"000";
                    
                    if(sharedVisits.multiVisitArrive) {
                        [sharedVisits.onSequenceArray removeObject:localVisit];
                    ...
                    //@synchronized (@"WriteSend") {
                        /*dispatch_queue_t writeVisitInfo = dispatch_queue_create("WriteCompleteState", NULL);
                        dispatch_async(writeVisitInfo, ^{
                            [localVisit writeVisitDataToFile];
                        });*/
                        [self markVisitArriveOrComplete:@"completed"
                                                  andAppointmentID:localVisit.appointmentid
                                                    andVisitDetail:localVisit
                                                        withButton:statusButton];
                    //}
...
-(void) markVisitArriveOrComplete:(NSString*)visitStatus
                andAppointmentID:(NSString*)appointmentID
                   andVisitDetail:(VisitDetails*)visit
                       withButton:(UIButton*)arriveCompleteButton {
    ...
    NSString *urlLoginStr = [[[NSUserDefaults standardUserDefaults]objectForKey:@"username"]
                             stringByAddingPercentEncodingWithAllowedCharacters:
                             NSCharacterSet.URLQueryAllowedCharacterSet];
    
    NSString *urlPassStr = [[[NSUserDefaults standardUserDefaults]objectForKey:@"password"]
                            stringByAddingPercentEncodingWithAllowedCharacters:
                            NSCharacterSet.URLQueryAllowedCharacterSet];
    ...
    if(sharedVisits.isReachable) {
        ...
        [request setValue:[sharedVisits getUserAgent] forHTTPHeaderField:@"User-Agent"];
        ...
        NSURLSessionDataTask *postDataTask = [urlSession dataTaskWithRequest:request
            ...
            if(error == nil) {
                ...
                if ([visitStatus isEqualToString:@"arrived"])
                {
                    ...
                    [[NSNotificationCenter defaultCenter]postNotificationName:@"sentMarkArrive" object:nil];
                ...
                else if ([visitStatus isEqualToString:@"completed"]) 
                {
                    ...
                    [[NSNotificationCenter defaultCenter]postNotificationName:@"sentVisitComplete" object:nil];
                    //[[NSNotificationCenter defaultCenter]postNotificationName:@"MarkComplete" object:nil];
                ...
                dispatch_async(myQueue, ^{
                    if([visitStatus isEqualToString:@"arrived"]) {
                        ...
                        visit.completed = dateTimeString;
                        visit.NSDateMarkComplete = rightNow;
                        visit.dateTimeMarkComplete = dateTimeString;
                        [visit markComplete:dateTimeString2 latitude:theLatitude longitude:theLongitude];
        ...
        /*NSURL *AFURL = [NSURL URLWithString:@"https://leashtime.com/native-visit-action.php"];
        NSMutableDictionary *bodyParams = [[NSMutableDictionary alloc]init];
        [bodyParams setObject:urlLoginStr forKey:@"loginid"];
        [bodyParams setObject:urlPassStr forKey:@"password"];
        [bodyParams setObject:dateTimeString forKey:@"datetime"];
        NSMutableDictionary *coordsDic = [[NSMutableDictionary alloc]init];
        [coordsDic setObject:appointmentID forKey:@"appointmentptr"];
        [coordsDic setObject:theLatitude forKey:@"lat"];
        [coordsDic setObject:theLongitude forKey:@"lon"];
        [coordsDic setObject:visitStatus forKey:@"event"];
        [coordsDic setObject:theAccuracy forKey:@"accuracy"];
        [bodyParams setObject:coordsDic forKey:@"coords"];
        NSLog(@"Body: %@", bodyParams);
        
        AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc]initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
        //manager.requestSerializer = [AFJSONRequestSerializer serializer];
        manager.requestSerializer = [AFHTTPRequestSerializer serializer];
        [manager.requestSerializer setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
        [manager.requestSerializer setValue:[sharedVisits getUserAgent] forHTTPHeaderField:@"User-Agent"];
        ...
        [manager POST:AFURL.absoluteString
...
// NETWORKING RELIABILITY AND RESEND AND OTHER UTILS
...
-(void) reloadTableView {
    NSLog(@"RELOAD TABLE VIEW CALLED");
...
-(BOOL) checkForBadResendBeforeSync {
    ...
    for(VisitDetails *visit in sharedVisits.visitData) {
        ...
        } else if ([visit.mapSnapUploadStatus isEqualToString:@"FAIL"]) {

            sendFail = YES;
            [visit resendMapSnapShot];
            
        } else if ([visit.visitReportUploadStatus isEqualToString:@"FAIL"]) {
            sendFail = YES;
            [self sendVisitReportNoButton:visit.appointmentid];
            
        } else if ([visit.currentCompleteVisitStatus isEqualToString:@"FAIL"]) {

            sendFail = YES;
            [self resendComplete:visit.appointmentid];
            
        } else if ([visit.currentArriveVisitStatus isEqualToString:@"FAIL"]) {
            sendFail = YES;
...
-(void) resendMarkArriveOrComplete:(VisitDetails*)visit toStatus:(NSString*)status {
    ...
    NSString *userAgentString = [sharedVisits getUserAgent];
...
-(void) showReachabilityIcon {
    ...
    if(!sharedVisits.isReachable) {
        __block UILabel *networkProblem = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
        ...
        [UIView animateWithDuration:3.0
            ...
            networkProblem.frame = CGRectMake(0,0, self.view.frame.size.width, 20);
        
        } completion:^(BOOL finished) {
            networkProblem.frame = CGRectMake(0, 0, 0, 0);
            [networkProblem removeFromSuperview];
            networkProblem = nil;
        }];
    }
